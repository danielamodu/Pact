  ...
  return (
    <>
      <nav className="fixed bottom-6 left-1/2 -translate-x-1/2 z-50 max-w-[calc(100vw-2rem)]">
        <div className="flex items-center gap-1.5 bg-forest/95 backdrop-blur-xl rounded-full px-3 py-3 shadow-[0_22px_60px_-14px_rgba(26,60,43,0.7)] border border-white/10">
          <Link
            href="/"
            className="w-12 h-12 rounded-full bg-white/10 hover:bg-white/20 transition-colors flex items-center justify-center flex-shrink-0"
            title="Pact home"
          >
            <PactLogo className="w-7 h-7" />
          </Link>
          ...
          <div className="hidden sm:flex items-center gap-1 px-1.5">
            {links.map((item) => (
              <Link
                key={item.id}
                href={item.href}
                className={`px-5 py-3 rounded-full text-[15px] font-medium transition-colors whitespace-nowrap ${
                  activeItem === item.id
          ...
          {publicAddress ? (
            <div className="flex items-center gap-1.5">
              <button
                ...
                className="bg-mint text-forest text-[15px] font-semibold px-5 py-3 rounded-full hover:brightness-105 transition-all cursor-pointer flex items-center gap-2 whitespace-nowrap"
              >
              ...
              <button
                ...
                title="Sign out"
                className="w-12 h-12 rounded-full text-white/60 hover:text-white hover:bg-white/10 transition-colors flex items-center justify-center cursor-pointer flex-shrink-0"
              >
            ...
            <Link
              href="/login"
              className="bg-mint text-forest text-[15px] font-semibold px-6 py-3 rounded-full hover:brightness-105 transition-all whitespace-nowrap"
            >
              Sign in
            </Link>
          )}
